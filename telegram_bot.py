...
# ================== SOZLAMALAR ==================
TOKEN = ""          
ADMIN_ID = 123456789  # Bu yerga o'z ID raqamingizni kiriting
...
def admin_cancel_order(call):
    ...
    if current_status == "cancelled":
        conn.close()
        bot.answer_callback_query(call.id, "Allaqachon bekor qilingan.", show_alert=True)
        return
    ...
    bot.send_message(call.message.chat.id, f"❌ Buyurtma #{order_id} bekor qilindi.")
...
def customer_cancel_request(call):
    ...
    bot.answer_callback_query(call.id, "Savollar uchun administratorga murojaat qiling")
    bot.send_message(
        call.message.chat.id,
        f"ℹ️ Buyurtma #{order_id}\n\n"
        f"Savollar uchun:\n"
        f"📞 Tel: {CONTACT_PHONE}"
    )
