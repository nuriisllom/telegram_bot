...
def my_orders(call):
    ...
    if not orders:
        ...
        for order_id, status, total, date in orders:
            status_emoji = {"new": "⏳", "preparing": "👨‍🍳", "delivering": "🚗", "delivered": "✅"}.get(status, "❓")
            text += f"{status_emoji} Buyurtma #{order_id} — {status}\n💰 {total} so'm | 📅 {date}\n\n"
...
def order_status_command(message):
    ...
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton("⏳ Yangi", callback_data=f"setstatus|{order_id}|new"),
        types.InlineKeyboardButton("‍🍳 Tayyorlanmoqda", callback_data=f"setstatus|{order_id}|preparing"),
    )
    markup.add(
        types.InlineKeyboardButton(" Yetkazilmoqda", callback_data=f"setstatus|{order_id}|delivering"),
        types.InlineKeyboardButton("✅ Yetkazildi", callback_data=f"setstatus|{order_id}|delivered"),
    )
...
def finalize_order(chat_id, user_id, payment_method):
    ...
    user_message = (
        ...
        f"Buyurtma holatini ko'rish uchun /status buyrug'ini yuboring."
    )
    bot.send_message(chat_id, user_message, reply_markup=types.ReplyKeyboardRemove())
    ...
    if ADMIN_ID and ADMIN_ID != 0:
        try:
            admin_message = (
                ...
                f"Holatni o'zgartirish: /order_{order_id}"
            )
...
def check_status(message):
    ...
    for order_id, status, date in orders:
        status_emoji = {"new": "⏳", "preparing": "👨‍🍳", "delivering": "🚗", "delivered": "✅"}.get(status, "")
        status_text += f"{status_emoji} Buyurtma #{order_id} — {status}\n📅 {date}\n\n"
